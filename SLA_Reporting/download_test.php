...
$letterarr = range('A', 'Z');
for ($i = 0; $i < count($tickets); $i++) {//ticket zahl
//     echo $i+1;
    echo $letterarr[$i];
    $objPHPExcel->getActiveSheet()->getColumnDimension($letterarr[$i])->setWidth(20);
    
        for($ii = 0; $ii < count($row_name); $ii++) { //0,1,2,3,4,5
            echo $ii;
//             echo $letterarr[$ii].($i+1).":". $tickets[$i][$key[$ii]] ."<br>";
            $objPHPExcel->getActiveSheet()->SetCellValue($letterarr[$ii]."1", $row_name[$ii]);
            $objPHPExcel->getActiveSheet()->SetCellValue($letterarr[$ii].($i+2), $tickets[$i][$key[$ii]]);
//             $objPHPExcel->getActiveSheet()->getStyle($letterarr[$ii] . "1")
